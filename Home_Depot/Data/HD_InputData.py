...
df_test = pd.read_csv('~/Desktop/Home Depot/Data/test.csv', encoding="ISO-8859-1")#[:1000]
df_pro_desc = pd.read_csv('~/Desktop/Home Depot/Data/product_descriptions.csv')#[:1000]
...
def spell_check(search):
    ...
    'ligth': 'light',
    'sissors': 'scissors', "undercabinet": "under cabinet",
    ...
    if search in spell_check_dict:
        print ('')
        print (search, 'to', spell_check_dict[search])
        search = (spell_check_dict[search])
    else:
        search = (search)
...
def get_similar(a, b):
    print ('Similarity: ', SequenceMatcher(None, a, b).ratio())
...
def get_levenshtein(a, b):
    ...
    print ('Levenshtein_Similarity: ', current[n])
    return current[n]

df_all['search_term'] = df_all['search_term'].map(lambda x:str_stem(spell_check(spell_correction(x))))
...
attributes = pd.read_csv('~/Desktop/Home Depot/Data/attributes.csv', encoding="ISO-8859-1")[['product_uid', 'value']]
attributes.fillna('', inplace=True)

attributestats = train.iloc[:][['id', 'product_uid', 'search_term']].copy()
attributestats = pd.merge(attributestats, attributes, how='left', on='product_uid')
attributestats.search_term = attributestats.apply(lambda x: get_cleantext(x['search_term']), axis=1)
attributestats.value = attributestats.apply(lambda x: get_cleantext(x['value']), axis=1)

attributestats['attributecosine'] = attributestats.apply(lambda x: get_cosine(text_to_vector(x['search_term']),text_to_vector(x['value'])), axis=1)
attributestats['attributeham'] = attributestats.apply(lambda x: get_hamdist(x['search_term'],x['value']), axis=1)
attributestats['attributesimilar'] = attributestats.apply(lambda x: get_similar(x['search_term'],x['value']), axis=1)
attributestats['attributelevenshtein'] = attributestats.apply(lambda x: get_levenshtein(x['search_term'],x['value']), axis=1)

attributestats = attributestats.groupby('id')[['attributecosine', 'attributeham', 'attributesimilar', 'attributelevenshtein']].sum().reset_index()
feature_cols = [col for col in attributestats.columns if col not in ['id']]
attributestats = attributestats[feature_cols]

df_all['cosine'] = attributestats['attributecosine']
df_all['hamdist'] = attributestats['attributeham']
df_all['similar'] = attributestats['attributesimilar']
df_all['levenshtein'] = attributestats['attributelevenshtein']

df_all.to_csv(os.path.expanduser('~/Desktop/Home Depot/Features/initial_features.csv'))
